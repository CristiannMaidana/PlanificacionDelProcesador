    ...
    public void run(){
        while (!done() || soPhase!=SOPhase.NONE || running!=null
            ...
            // CORRIENDO -> BLOQUEADO
                ...
                running.setCurPriority(running.getPriority());      // prioridad base para la nueva ráfaga
                running = null; burstJustFinished = false; qRemaining = 0;
            }
            ...
            if (soPhase==SOPhase.NONE && running!=null && (wantPreempt || (useQuantum && quantumExpired))) {
                ...
                ready.add(running);
                running = null; wantPreempt = false; quantumExpired = false;
                ...
                if (nextDispatch != null) {
                    if (OS.getTCP() <= 0) {
                        ...
                        log("t=%d | LISTO→CORRIENDO (TCP=0): %s (prio=%d)", t, running.getProcessName(), running.getCurPriority());
                        nextDispatch = null;
                    } else {
            ...
            if (soPhase != SOPhase.NONE) {
                ...
                if (useQuantum && qRemaining > 0) qRemaining--;
                if (useAging) {
