...
public class SRTNPlanner extends SimulatorStrategy{

    final PriorityQueue<Process> ready = new PriorityQueue<>(
            Comparator.comparingInt((Process p)->p.getTimerBurstCPU()).thenComparingInt(p->p.getArrivalTime())
    );

    SRTNPlanner(OperatingSystem os, List<Process> batch){
        super(os, batch);
    }
    ...
    public void run(){
        while (!done() || soPhase!=SOPhase.NONE || running!=null
            ...
            if (soPhase==SOPhase.NONE && running!=null && wantPreempt) {
                log("t=%d | CORRIENDO→LISTO (expropiación SRTN): %s", t, running.getProcessName());
                ready.add(running);
                running = null; cpuRemaining = 0; wantPreempt = false;
            ...
            if (tfpJustFinished && finishingProcess != null) {
                M.onFinish(finishingProcess, t + 1);  // sólo cuando TFP>0
                finishingProcess = null; tfpJustFinished = false;
            }

            if (soPhase==SOPhase.NONE && running!=null && !ready.isEmpty()) {
                Process best = ready.peek();
                if (best != null && best.getTimerBurstCPU() < cpuRemaining) {
                    wantPreempt = true;
                } else {
                    wantPreempt = false;
                }
